...
namespace org.jiechan.service {
    public class FetchContent {
        ...
        public static string GetTitleFromUrl(string url) {
            ...
            string html = FetchContent.GetHTMLFromUrl(url);
            return GetTitleFromHTML(result);
        }

        /// <summary>
        /// 推荐用这个方法,省去了获取HTML的步骤,是比较节省资源的方式。
        /// </summary>
        ...
        public static string GetTitleFromHTML(string html) {
            ...
            if (indexof > 0) {
                result = result.Remove(indexof, result.Length - indexof);
            }

            result = RegexHelper.regReplace(result, "_.*", "");
            result = RegexHelper.regReplace(result, "-.*", "");
        ...
        /// <summary>
        /// 从URL中获取当前页面的keywords。
        /// </summary>
        /// <param name="html">网页的HTML文本</param>
        /// <returns>keywords标签中的关键字</returns>
        public static string GetKeyWordsFromHTML(string html) {
            string re = "";
            try {
                //虽然想法不错，但内容中的关键词，不是很理想，所以这里就去掉了。2013年12月18日。
        ...
        public static string GetHTMLFromUrl(string url) {
            EchoHelper.Echo("尝试链接：" + url + "，请稍等...", "内容下载", EchoHelper.EchoType.绿色信息);
        ...
        /// <img src = "http://xxxx.com.com/img/1.jgp"></img>
        /// </summary>
        /// <param name="tmp_content"></param>
        /// <param name="url"></param>
        public static void ImageSrc(ref string content, string url) {
            //如果遇到使用绝对路径的图片，转换为全路径。
            NodeList htmlNodes = new Parser(new Lexer(content)).Parse(new TagNameFilter("img"));
            ...
            for (int j = htmlNodes.Count - 1; j >= 0; j--) {
                ...
                if (!string.IsNullOrEmpty(urlpart) && !new Regex(@"^http:").IsMatch(urlpart)) {
                    ...
                    urlpart = new RzHttp().getDealUrl(url, urlpart);
        ...
        /// <summary>
        /// 把Link标签中的href属性，自动补全为带http的绝对路径。
        /// </summary>
        /// <param name="content"></param>
        /// <param name="url"></param>
        public static void LinkHref(ref string content, string url) {
            //如果遇到使用绝对路径的图片，转换为全路径。
            NodeList htmlNodes = new Parser(new Lexer(content)).Parse(new TagNameFilter("link"));

            for (int j = htmlNodes.Count - 1; j >= 0; j--) {
                LinkTag link = (LinkTag)htmlNodes.ElementAt(j);

                string urlpart = link.GetAttribute("href");
                ...
                if (!string.IsNullOrEmpty(urlpart) && !new Regex(@"^http:").IsMatch(urlpart)) {
                    ...
                    string newsrc = "href=\"" + urlpart + "\" ";
                    string newlink = link.ToHtml();

                    newlink = newlink.Insert(6, newsrc);
        ...
        public static void AHref(ref string content, string url) {
            //如果遇到使用绝对路径的图片，转换为全路径。
            NodeList htmlNodes = new Parser(new Lexer(content)).Parse(new TagNameFilter("a"));

            for (int j = htmlNodes.Count - 1; j >= 0; j--) {
                ATag link = (ATag)htmlNodes.ElementAt(j);
                ...
                if (!string.IsNullOrEmpty(urlpart) && !new Regex(@"^http:").IsMatch(urlpart)) {
                    ...
                    string newsrc = "href=\"" + urlpart + "\" ";
                    string newlink = link.ToHtml();
                    newlink = newlink.Insert(3, newsrc);
                    content = content.Replace(oldlink, newlink);
                    //EchoHelper.Echo("成功转换了一个图片的SRC属性！", "", EchoHelper.EchoType.普通信息);
                }
                if (!string.IsNullOrEmpty(urlpart) && new Regex(@"^\.\.").IsMatch(urlpart)) {
        ...
        /// <param name="content"></param>
        /// <param name="url"></param>
        public static void ScriptSrc(ref string content, string url) {
            //如果遇到使用绝对路径的图片，转换为全路径。
            NodeList htmlNodes = new Parser(new Lexer(content)).Parse(new TagNameFilter("script"));

            for (int j = htmlNodes.Count - 1; j >= 0; j--) {
                ScriptTag link = (ScriptTag)htmlNodes.ElementAt(j);
                ...
                if (!string.IsNullOrEmpty(urlpart) && !new Regex(@"^http:").IsMatch(urlpart)) {
                    ...
                    urlpart = new RzHttp().getDealUrl(url, urlpart);
                    string tmpold = link.ToHtml();
                    link.SetAttribute("src", urlpart);

                    content = content.Replace(tmpold, link.ToHtml());
                    //EchoHelper.Echo("成功转换了一个图片的SRC属性！", "", EchoHelper.EchoType.普通信息);
                }
                if (!string.IsNullOrEmpty(urlpart) && new Regex(@"^\.\.").IsMatch(urlpart)) {
                    string oldlink = link.ToHtml();
        ...
        /// <summary>
        /// 把完整的link标签，传入，返回href，一般都是css的href。
        /// </summary>
        /// <param name="html"></param>
        /// <returns></returns>
        public static string getLinkHref(string html) {
            LinkTag tmptag = new LinkTag();
            tmptag.SetText(html);
            string href = tmptag.GetAttribute("href");
            return href;
        }

        /// <summary>
        /// 把完整的Scrtip标签，传入，返回src。
        /// </summary>
        /// <param name="html"></param>
        /// <returns></returns>
        public static string getScriptSrc(string html) {
            ScriptTag tmptag = new ScriptTag();
            tmptag.SetText(html);
            string src = tmptag.GetAttribute("src");
            return src;
        }

        /// <summary>
        /// 获取Img标签中的src，传入的是完整的《Img》标签内容。
        /// </summary>
        /// <param name="html"></param>
        /// <returns></returns>
        public static string GetImageSrc(string html) {
            ImageTag tmptag = new ImageTag();
            tmptag.SetText(html);
            string src = tmptag.GetAttribute("src");
            return src;
        }

        public static string GetAHref(string html) {
            ATag tmptag = new ATag();
            tmptag.SetText(html);
            string src = tmptag.GetAttribute("href");
            return src;
        }
...
//         /// 内容过滤
//         /// </summary>
//         /// <param name="tmp_title"></param>
//         /// <param name="tmp_content"></param>
//         /// <param name="strreps"></param>
//         /// <param name="noimg"></param>
//         /// <param name="no2br"></param>
//         public static void Filter(ref model.ModelArticle ar, string strreps, bool noimg, bool no2br) {
//             //内容过滤循环
//             string strrep = "<!--.*?-->→\n";
//             strrep += "<div.*?>→\n";
//             strrep += "</div>→\n";
...
//                 if (!noimg) { //定义图片的集合，防止被正则过滤掉
//                     NodeList htmlNodesImgs = new Parser(new Lexer(ar.Content)).Parse(new TagNameFilter("img"));
// 
//                     for (int j = 0; j < htmlNodesImgs.Count; j++) {
//                         ImageTag img = (ImageTag)htmlNodesImgs.ElementAt(j);
//                         alimg.Add(img);
//                         ar.Content = ar.Content.Replace(img.ToHtml(), "∫" + j);
//                     }
//                 }
// 
// 
...
//                         ar.Title = ar.Title.Replace(tmpreg, "");
//                         ar.Content = ar.Content.Replace(tmpreg, "");
//                     } else if (!string.IsNullOrEmpty(tmpreg)) {
//                         string tmp01 = tmpreg.Split('→')[0].ToString();
//                         string tmp02 = tmpreg.Split('→')[1].ToString();
//                         ar.Title = RegexHelper.regReplaces(ar.Title, tmp01, tmp02);
//                         ar.Content = RegexHelper.regReplaces(ar.Content, tmp01, tmp02);
//                     }
//                 }
// 
//                 if (!noimg) { //定义图片的集合，防止被正则过滤掉
...
// 
//                         //针对新浪博客写的
//                         if (null != img.GetAttribute("REAL_SRC")) {
//                             img.ImageURL = img.Attributes["REAL_SRC"].ToString();
//                             img.RemoveAttribute("REAL_SRC");
//                         }
//                         ar.Content = ar.Content.Replace("∫" + j, "<br />" + img.ToHtml().Replace("\n", "").Replace("\r", "") + "<br />");
//                     }
//                 }
//             }
// 
//             ar.Title = StringHelper.FormatTitle(ar.Title).Replace("(组图", "").Replace("(图", "");
//             if (no2br) {
//                 ar.Content = StringHelper.FormatHtml(ar.Content);
//             } else {
//                 ar.Content = StringHelper.NoHtml(ar.Content);
//             }
// 
//         }
        ...
        private static void ModifyRawText(ref string rawtext) {
            string[] aryReg ={
        ...
        private static string GetMainContent(string html) {
            EchoHelper.Echo("哇，好大的页面，很努力的为你寻找正文内容ing，请稍等...", null, EchoHelper.EchoType.淡蓝信息);
            string reg1 = @"<(p|br)[^<]*>";
            ...
            EchoHelper.Echo("真不容易哇，提取成功了。...", "正文提取", EchoHelper.EchoType.淡蓝信息);
            return html;
        }
        ...
        public static void GetContentFromUrl(string url, ref string title, ref string content) {
            ...
            try {
                ...
                title = "";
                content = "";
                EchoHelper.Echo("采集跳过，原因可能是：该文章设置了密码、被删除、乱码等。", "采集出错", EchoHelper.EchoType.淡蓝信息);
            }
            EchoHelper.EchoEnd();
        }
        ...
        public static void ItemRetrival_1(string url, ref Encoding encode, ref List<string> listUrl, ref List<string> listTitle,
            ...
            //获取网页源码；
            string rawtext = GetHTMLFromUrl(url);
